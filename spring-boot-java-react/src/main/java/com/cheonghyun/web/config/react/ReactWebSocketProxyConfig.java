...
@Configuration
public class ReactWebSocketProxyConfig {
    private final Logger log = LoggerFactory.getLogger(ReactWebSocketProxyConfig.class);
    private final ReactProperties reactProperties;

    public ReactWebSocketProxyConfig(ReactProperties reactProperties) {
    ...
    private void handleWebSocketProxy(HttpServerExchange exchange) {
        try {
            ...
            try (DefaultByteBufferPool pool = new DefaultByteBufferPool(true, 1024)) {
                ...
                String scheme = Boolean.TRUE.equals(reactProperties.getSsl()) ? "wss" : "ws";
                String hostUrl = scheme + "://" + reactProperties.getHost() + ":" + reactProperties.getPort();
